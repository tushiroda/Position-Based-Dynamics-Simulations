...
particles = [
    Particle(i * particle_radii * 2.1, screen_topy + 3 * particle_radii)
    for i in range(15)
]

point_constraints = [PointConstraint(0, 0.0, screen_topy + 3 * particle_radii)]


distance_constraints = [
    Constraint(i, i + 1, particle_distance) for i in range(len(particles) - 1)
]
...
def distance_constraint(particle1, particle2, constraint_distance):
    ...
    if dist > constraint_distance:
        correction_x1 = tensileStrength * dx * (dist - constraint_distance)
        correction_y1 = tensileStrength * dy * (dist - constraint_distance)
...
def pbd_main_loop():
    ...
    while i < 4:
        ...
        for constraint in point_constraints:
            delta_x1, delta_y1 = point_constraint(
                particles[constraint.id1], constraint.x, constraint.y
            )
        ...
        for constraint in distance_constraints:
            ...
            delta_x1, delta_y1, delta_x2, delta_y2 = distance_constraint(
                particles[constraint.id1],
                particles[constraint.id2],
                constraint.distance,
            )
...
window = glfw.create_window(
    screen_dimx, screen_dimy, "White Square on Black Background", None, None
)
if not window:
    glfw.terminate()
    exit()
...
gluOrtho2D(screen_leftx, screen_rightx, screen_bottomy, screen_topy)
